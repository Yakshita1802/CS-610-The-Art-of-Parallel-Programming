...
Yakshita Rakholiya
Project-5 @Dr.Lixin Tao @Kai Wang
*/
...
#define numiter 2*d /*total iteration before termination*/ 
#define tolerance .1 
typedef float rowtype[n+2]; 
rowtype A[n+2]; /*main data of array*/ 
rowtype stream upchan[n+1], stream downchan[n+1];
int GrayCode[n+1]; 
int i; 
boolean stream inchan[n][d+1]; /*for Aggregation*/ 

boolean Aggregate_fun(boolean mydone) { 
  /*Multiple Aggregation function from Fig 8.11*/ 
  return true;
} 
   
void Rowupdation(int me, value rowtype myrow, rowtype out) { 
  int j, k; 
  ...
  do { 
    for (k=1; k<=numiter; k++) { 
	  ...
	  if (me > 1) 
		send(upchan[me-1], myrow);
	  if (me < n) { 
	    send(downchan[me+1], myrow);
		recv(upchan[me], uprow);
	  }
	  if (me > 1) 
		recv(downchan[me], downrow); 
	  maxchange = 0;
      for (j = 1; j <= n; j++) { 
	...
	/*termination test*/ 
	done = Aggregate_fun(maxchange < tolerance);
  } while (!done); 
  out = myrow; 
}

/* generate dim-bit gray code in array GrayCode[] */
void Gray_computation(int dim) {
  int len;  
  int bits; 
  int base;  
  int i;
...
main( ) { 
  ...
  Gray_computation(d);  /*Initialize Gray Code array for Hypercube*/ 
  ...
  forall i = 1 to n 
    @GrayCode[i] do Rowupdation(i, A[i], A[i]); 
